...
END $$;
-- ============================================================================
-- Delta: Store access control (which stores an account may sign in from)
-- Idempotent: safe to re-run.
-- ============================================================================

-- 1. Allowed login stores per account (NULL = any store)
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS allowed_store_ids bigint[];

-- 2. Create user carries allowed stores
...
-- 3. Update user carries allowed stores (NULL = leave unchanged, {} = any store)
...
-- 4. User list carries allowed stores
CREATE OR REPLACE FUNCTION public.app_get_users()
RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
AS $$
