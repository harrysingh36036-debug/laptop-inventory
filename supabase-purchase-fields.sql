-- ---------------------------------------------------------------------------
-- Delta: purchase record fields — charger (with/without), mandatory comment,
--   hashed purchaser Aadhar (admin-only visibility).
-- Applies after supabase-product-line-ram.sql
-- Run once: psql "$DATABASE_URL" -f supabase-purchase-fields.sql
-- ---------------------------------------------------------------------------
...
-- Normalize: carry the new purchase-record fields
...
BEGIN
  ...
  IF COALESCE(v_l->>'purchase_comment','') = '' THEN RAISE EXCEPTION 'purchase_comment is required'; END IF;
...
BEGIN
  ...
  IF COALESCE(NULLIF(btrim(COALESCE(p_data->>'purchase_comment','')), ''), '') = '' THEN RAISE EXCEPTION 'purchase_comment is required'; END IF;
...
-- Update: set the new columns (comment required on edit too)
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'charger', l.charger, 'purchase_comment', l.purchase_comment, 'purchaser_aadhar_hash', l.purchaser_aadhar_hash,
