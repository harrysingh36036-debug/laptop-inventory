-- ---------------------------------------------------------------------------
-- Delta: product_line + ram for laptop specs
-- Applies after supabase-v3-master.sql (inventory form now uses:
--   Brand | Product line | Model | Processor | RAM | SSD/storage)
-- Run once: psql "$DATABASE_URL" -f supabase-product-line-ram.sql
-- ---------------------------------------------------------------------------
...
-- Normalize: carry the two new spec fields
...
BEGIN
  ...
  END IF;
  INSERT INTO public.laptops (brand, brand_model, product_line, processor_type, ram, generation, storage_type, storage_size, purchased_from, graphics, graphics_type, graphics_model, purchase_rate, extra_charges, serial_number, current_store_id, status)
  VALUES (v_l->>'brand', v_l->>'brand_model', v_l->>'product_line', v_l->>'processor_type', v_l->>'ram', v_l->>'generation', v_l->>'storage_type', v_l->>'storage_size', v_l->>'purchased_from', v_l->>'graphics', v_l->>'graphics_type', v_l->>'graphics_model', (v_l->>'purchase_rate')::numeric, (v_l->>'extra_charges')::numeric, v_serial, (v_l->>'current_store_id')::bigint, v_l->>'status')
...
-- Listing + search: expose the new fields
