-- ============================================================================
-- Delta: Store raw Aadhar for admin reveal
--  1. Add purchaser_aadhar column to laptops + purchases tables
--  2. Update app_get_laptops to return raw aadhar only for admins
--  3. Update app_get_purchases to return raw aadhar only for admins
--  4. Update app_normalize_laptop, app_create_laptop, app_bulk_create_laptops,
--     app_update_laptop to handle the new column
--  5. Update app_create_purchase, app_update_purchase to handle the new column
-- Idempotent: safe to re-run.
-- ============================================================================

-- 1. Add column to laptops table
ALTER TABLE public.laptops ADD COLUMN IF NOT EXISTS purchaser_aadhar text;
COMMENT ON COLUMN public.laptops.purchaser_aadhar IS 'Raw 12-digit Aadhar — visible to admin/superadmin only';

-- Add column to purchases table
ALTER TABLE public.purchases ADD COLUMN IF NOT EXISTS purchaser_aadhar text;

-- 2. Update app_get_laptops to return raw aadhar for admins
...
-- 3. Update app_get_purchases to return raw aadhar for admins
...
-- 4. Update app_normalize_laptop to include purchaser_aadhar
...
-- 5. Update app_create_laptop to store purchaser_aadhar
...
-- 6. Update app_bulk_create_laptops to store purchaser_aadhar
...
-- 7. Update app_update_laptop to store purchaser_aadhar
...
-- 8. Update app_create_purchase to store purchaser_aadhar
...
-- 9. Update app_update_purchase to store purchaser_aadhar
